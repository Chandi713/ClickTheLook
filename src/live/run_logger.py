...
# Logs land at <project_root>/logs/
_LOG_DIR = Path(__file__).resolve().parents[2] / "logs"

# Timeline is sampled every N frames (keeps JSON size manageable)
...
class RunLogger:
    ...
    def __init__(
        ...
        *,
        ...
        # ── Per-frame accumulators (plain lists for O(1) append) ────────────
        ...
        self._conf_scores:  list[float] = []   # raw confidence values

        self._class_det_counts: dict[str, int]   = {}  # class -> total detections
        self._class_trk_counts: dict[str, int]   = {}  # class -> frames tracked
        self._track_registry:   dict[int, dict]  = {}  # id -> {class, first, last}

        self._timeline: list[dict] = []
        self._frame_anomalies: list[dict] = []  # frames flagged as unusual
    ...
    def log_frame(
        ...
        tracked: np.ndarray,      # (M, 9)  [x1,y1,x2,y2,cls,_,_,_,id]
        class_names: dict,
    ) -> None:
        ...
        # Track registry + per-class tracking counts
        if n_tracks > 0:
            for row in tracked:
                tid  = int(row[8])
                cid  = int(row[4])
                ...
                if tid not in self._track_registry:
                    ...
                    # Count frames where det=0 but track survived (Kalman prediction)
    ...
    def finalize(self, frames_processed: int, total_elapsed: float) -> str:
        ...
        lifespans = [
            v["last_frame"] - v["first_frame"] + 1
            for v in self._track_registry.values()
        ]
        ...
        m["anomalies"] = {
            ...
            "slow_frames":                   slow_frames[:50],  # cap at 50
    ...
    def _consecutive_zero_det_sequences(self) -> list[dict]:
        ...
        for i, n in enumerate(self._dets_per_frame):
            if n == 0 and not in_seq:
                ...
                in_seq = False
        if in_seq:
            seqs.append({"start_frame": start, "end_frame": len(self._dets_per_frame),
                         "length": len(self._dets_per_frame) - start + 1})
        return sorted(seqs, key=lambda x: x["length"], reverse=True)[:20]


# ── Helper: build model_info dict from a YOLO instance ──────────────────────
